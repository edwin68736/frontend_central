...
function queueWithFailed(items: Array<Record<string, unknown>>) {
  return {
    queued: [],
    queued_count: 0,
    processing: [],
...
async function setup(items: Array<Record<string, unknown>>) {
  ...
  mocks.getTenants.mockResolvedValue({ items: [], total: 0 })
  mocks.getQueue.mockResolvedValue(queueWithFailed(items))
